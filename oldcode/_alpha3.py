...
class Alpha3():
    ...
    def compute_meta_info(self,trade_range):
        ...
        for inst in self.insts:
            ...
            fast = np.where(inst_df.close.rolling(10).mean() > inst_df.close.rolling(20).mean(), 1, 0)
    ...
    def run_simulation(self):
        ...
        for i in portfolio_df.index:
            ...
            nominal_tot = 0
            aboslute_scores = np.abs([score for score in alpha_scores.values()])
            forecast_chips = np.sum(aboslute_scores)
            for inst in eligibles:
